...
function ConstructorRow({ team, pct, isTop3, trigger }) {
  const points = useCountUp(parseFloat(team.points), trigger);
  ...
  return (
    <div className="standing-row rd-row-hover flex items-center gap-4 py-2.5 border-b border-[var(--rd-line)]">
      ...
      {logoUrl && <img src={logoUrl} alt={team.Constructor.name} className="w-4 h-4 object-contain shrink-0" loading="lazy" />}
      <div className="flex-1 min-w-0">
        <span className="text-[var(--rd-text)] font-medium text-sm">{team.Constructor.name}</span>
...
function ConstructorStandingsV2() {
  ...
  return (
    <div ref={listRef}>
      <h2 className="rd-display text-2xl sm:text-3xl mb-6 pb-4 border-b border-[var(--rd-line)]">Constructor Standings</h2>
      ...
      {standings.length === 0 ? (
        ...
        <div ref={revealRef}>
          {standings.map((team, i) => {
            ...
            return (
              <ConstructorRow key={team.Constructor.constructorId} team={team} pct={pct} isTop3={i < 3} trigger={visible} />
            );
          })}
        </div>
      )}
...
export default ConstructorStandingsV2;
